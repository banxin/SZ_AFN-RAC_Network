...
// 是否为空对象
#define SZObjectIsNil(__object)  ((nil == __object) || [__object isKindOfClass:[NSNull class]])
// 字符串为空
#define SZStringIsEmpty(__string) ((__string.length == 0) || SZObjectIsNil(__string))
// 字符串不为空
#define SZStringIsNotEmpty(__string)  (!SZStringIsEmpty(__string))
...
NSString *const SZHTTPServiceErrorDomain = @"SZHTTPServiceErrorDomain";
/// 请求成功，但statusCode != 0
NSString *const SZHTTPServiceErrorResponseCodeKey = @"SZHTTPServiceErrorResponseCodeKey";


NSString * const SZHTTPServiceErrorRequestURLKey = @"SZHTTPServiceErrorRequestURLKey";
NSString * const SZHTTPServiceErrorHTTPStatusCodeKey = @"SZHTTPServiceErrorHTTPStatusCodeKey";
NSString * const SZHTTPServiceErrorDescriptionKey = @"SZHTTPServiceErrorDescriptionKey";
NSString * const SZHTTPServiceErrorMessagesKey = @"SZHTTPServiceErrorMessagesKey";


/// 连接服务器失败 default
NSInteger const SZHTTPServiceErrorConnectionFailed = 668;
NSInteger const SZHTTPServiceErrorJSONParsingFailed = 669;

NSInteger const SZHTTPServiceErrorBadRequest = 670;
NSInteger const SZHTTPServiceErrorRequestForbidden = 671;
/// 服务器请求失败
NSInteger const SZHTTPServiceErrorServiceRequestFailed = 672;
NSInteger const SZHTTPServiceErrorSecureConnectionFailed = 673;

@implementation SZHTTPService
...
#pragma mark - Request
- (RACSignal *) enqueueRequest:(SZHTTPRequest *) request resultClass:(Class /*subclass of MHObject*/) resultClass
{
    /// request 必须的有值
    if (!request) return [RACSignal error:[NSError errorWithDomain:SZHTTPServiceErrorDomain code:-1 userInfo:nil]];
    ...
    return [[[self enqueueRequestWithPath:request.urlParameters.path parameters:request.urlParameters.parameters method:request.urlParameters.method]
             reduceEach:^RACStream *(NSURLResponse *response, NSDictionary * responseObject){
                 ...
                 return [[self parsedResponseOfClass:resultClass fromJSON:responseObject]
                         map:^(id parsedResult) {
                             SZHTTPResponse *parsedResponse = [[SZHTTPResponse alloc] initWithResponseObject:responseObject parsedResult:parsedResult];
                             NSAssert(parsedResponse != nil, @"Could not create SZHTTPResponse with response %@ and parsedResult %@", response, parsedResult);
...
#pragma clang diagnostic pop
        ...
        task = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, NSDictionary * responseObject, NSError *error) {
            if (error) {
                ...
                /// 在这里判断数据是否正确
                /// 判断
                NSInteger statusCode = [responseObject[SZHTTPServiceResponseCodeKey] integerValue];
                if (statusCode == SZHTTPResponseCodeSuccess) {
                    
                    //                    NSHTTPURLResponse *httpUrlResponse = (NSHTTPURLResponse *)response;
                    //                    /// 存储token
                    //                    NSString *token = [[[httpUrlResponse allHeaderFields] valueForKey:SZHTTPRequestTokenKey] sz_stringValueExtension];
                    ...
                    if (statusCode == SZHTTPResponseCodeNotLogin) {
                        /// 需要登录
                        //                        [self login:^{
                        //                            /// 这里需要重新配置序列化
                        //                            self.requestSerializer = [self _requestSerializerWithRequest:[SZHTTPRequest requestWithParameters:[MHURLParameters urlParametersWithMethod:method path:path parameters:parameters]]];
                        //                            /// 重新发起请求
                        //                            [[self enqueueRequestWithPath:path parameters:parameters method:method] subscribe:subscriber];
                        //                        } cancel:^{
                        //                            NSMutableDictionary *userInfo = [NSMutableDictionary dictionary];
                        //                            userInfo[SZHTTPServiceErrorResponseCodeKey] = @(statusCode);
                        //                            NSString *msgTips = responseObject[SZHTTPServiceResponseMsgKey];
                        //#if defined(DEBUG)||defined(_DEBUG)
                        //                            msgTips = SZStringIsNotEmpty(msgTips)?[NSString stringWithFormat:@"%@(%zd)",msgTips,statusCode]:[NSString stringWithFormat:@"服务器出错了，请稍后重试(%zd)~",statusCode];                 /// 调试模式
                        //#else
                        //                            msgTips = SZStringIsNotEmpty(msgTips)?msgTips:@"服务器出错了，请稍后重试~"; /// 发布模式
                        //#endif
                        //                            userInfo[SZHTTPServiceErrorMessagesKey] = msgTips;
                        //                            if (task.currentRequest.URL != nil) userInfo[SZHTTPServiceErrorRequestURLKey] = task.currentRequest.URL.absoluteString;
                        //                            if (task.error != nil) userInfo[NSUnderlyingErrorKey] = task.error;
                        //                            NSError *requestError = [NSError errorWithDomain:SZHTTPServiceErrorDomain code:statusCode userInfo:userInfo];
...
#pragma mark - Upload
- (RACSignal *)enqueueUploadRequest:(SZHTTPRequest *)request resultClass:(Class)resultClass fileDatas:(NSArray<NSData *> *)fileDatas name:(NSString *)name mimeType:(NSString *)mimeType
{
    /// request 必须的有值
    if (!request) return [RACSignal error:[NSError errorWithDomain:SZHTTPServiceErrorDomain code:-1 userInfo:nil]];
    /// 断言
    NSAssert(SZStringIsNotEmpty(name), @"name is empty: %@", name);
    ...
    return [[[self enqueueUploadRequestWithPath:request.urlParameters.path parameters:request.urlParameters.parameters constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        for (int i = 0; i< count; i++) {
            ...
            [formData appendPartWithFileData:fileData name:name fileName:fileName mimeType:SZStringIsNotEmpty(mimeType)?mimeType:@"application/octet-stream"];
        }
    }]
             reduceEach:^RACStream *(NSURLResponse *response, NSDictionary * responseObject){
                 @strongify(self);
            ...
            if (error) {
                ...
                if (statusCode == SZHTTPResponseCodeSuccess) {
                    ...
                    if (statusCode == SZHTTPResponseCodeNotLogin) {
                        ...
                        //                        [self login:^{
                        //                            /// 这里需要重新配置序列化
                        //                            self.requestSerializer = [self _requestSerializerWithRequest:[SZHTTPRequest requestWithParameters:[MHURLParameters urlParametersWithMethod:@"POST" path:path parameters:parameters]]];
                        //                            /// 重新发起请求
                        ...
                        //                            if (task.currentRequest.URL != nil) userInfo[SZHTTPServiceErrorRequestURLKey] = task.currentRequest.URL.absoluteString;
                        //                            if (task.error != nil) userInfo[NSUnderlyingErrorKey] = task.error;
                        //                            [subscriber sendError:[NSError errorWithDomain:SZHTTPServiceErrorDomain code:statusCode userInfo:userInfo]];
                        //                        }];
                    }else{
...
- (NSError *)parsingErrorWithFailureReason:(NSString *)localizedFailureReason
{
    ...
    return [NSError errorWithDomain:SZHTTPServiceErrorDomain code:SZHTTPServiceErrorJSONParsingFailed userInfo:userInfo];
}
...
- (RACSignal *)parsedResponseOfClass:(Class)resultClass fromJSON:(NSDictionary *)responseObject
{
    ...
    NSParameterAssert((resultClass == nil || [resultClass isSubclassOfClass:SZBaseModel.class]));
    
    /// 这里主要解析的是 data:对应的数据
    responseObject = responseObject[SZHTTPServiceResponseDataKey];
    ...
    return [RACSignal createSignal:^ id (id<RACSubscriber> subscriber) {
        ...
        void (^parseJSONDictionary)(NSDictionary *) = ^(NSDictionary *JSONDictionary) {
            ...
            /// 这里继续取出数据 data{"list":[]}
            NSArray * JSONArray = JSONDictionary[SZHTTPServiceResponseDataListKey];
            if ([JSONArray isKindOfClass:[NSArray class]]) {
                /// 字典数组 转对应的模型
                NSArray *parsedObjects = [NSArray modelArrayWithClass:resultClass.class json:JSONArray];
                /// 这里还需要解析是否是MHObject的子类
                for (id parsedObject in parsedObjects) {
                    /// 确保解析出来的类 也是 MHObject
                    NSAssert([parsedObject isKindOfClass:SZBaseModel.class], @"Parsed model object is not an MHObject: %@", parsedObject);
                }
                [subscriber sendNext:parsedObjects];
                
            }else{
                /// 字典转模型
                SZBaseModel *parsedObject = [resultClass modelWithDictionary:JSONDictionary];
                if (parsedObject == nil) {
                ...
                NSAssert([parsedObject isKindOfClass:SZBaseModel.class], @"Parsed model object is not an BaseModel: %@", parsedObject);
                /// 发送数据
...
- (NSError *)_errorFromRequestWithTask:(NSURLSessionTask *)task httpResponse:(NSHTTPURLResponse *)httpResponse responseObject:(NSDictionary *)responseObject error:(NSError *)error
{
    ...
    NSMutableDictionary *userInfo = [NSMutableDictionary dictionary];
    /// default errorCode is SZHTTPServiceErrorConnectionFailed，意味着连接不上服务器
    NSInteger errorCode = SZHTTPServiceErrorConnectionFailed;
    ...
    switch (HTTPCode) {
        case 400:{
            errorCode = SZHTTPServiceErrorBadRequest;           /// 请求失败
            break;
        }
        case 403:{
            errorCode = SZHTTPServiceErrorRequestForbidden;     /// 服务器拒绝请求
            break;
        }
        case 422:{
            errorCode = SZHTTPServiceErrorServiceRequestFailed; /// 请求出错
            break;
        }
        default:
            /// 从error中解析
            if ([error.domain isEqual:NSURLErrorDomain]) {
                ...
                switch (error.code) {
                    ...
                    case NSURLErrorClientCertificateRequired:
                        errorCode = SZHTTPServiceErrorSecureConnectionFailed; /// 建立安全连接出错了
                        break;
    ...
    userInfo[SZHTTPServiceErrorHTTPStatusCodeKey] = @(HTTPCode);
    userInfo[SZHTTPServiceErrorDescriptionKey] = errorDesc;
    if (task.currentRequest.URL != nil) userInfo[SZHTTPServiceErrorRequestURLKey] = task.currentRequest.URL.absoluteString;
    if (task.error != nil) userInfo[NSUnderlyingErrorKey] = task.error;
    return [NSError errorWithDomain:SZHTTPServiceErrorDomain code:errorCode userInfo:userInfo];
}
...
/// 基础的请求参数
- (NSMutableDictionary *)_parametersWithRequest:(SZHTTPRequest *)request
{
...
- (NSString *)_signWithParameters:(NSDictionary *) parameters
{
    ...
    for (id key in sortedKeys) {
        /// value 为 empty 跳过
        if(SZObjectIsNil(parameters[key])) continue;
        NSString * value = [parameters[key] sz_stringValueExtension];
        if (SZObjectIsNil(value)||!SZStringIsNotEmpty(value)) continue;
        value = [value sz_removeBothEndsWhitespaceAndNewline];
        value = [value sz_URLEncoding];
        [kvs addObject:[NSString stringWithFormat:@"%@=%@",key,value]];
    }
    /// 拼接私钥
    NSString *paramString = [kvs componentsJoinedByString:@"&"];
    NSString *keyValue = SZHTTPServiceKeyValue;
    NSString *signedString = [NSString stringWithFormat:@"%@&%@=%@",paramString,SZHTTPServiceKey,keyValue];
...
/// 序列化
- (AFHTTPRequestSerializer *)_requestSerializerWithRequest:(SZHTTPRequest *) request
{
    /// 获取基础参数（参数+拓展参数）
    ...
    /// 赋值
    parameters[SZHTTPServiceSignKey] = [sign length]?sign:@"";
    /// 请求序列化
    AFHTTPRequestSerializer *requestSerializer = [AFHTTPRequestSerializer serializer];
    /// 配置请求头
    for (NSString *key in parameters) {
        NSString *value = [[parameters[key] sz_stringValueExtension] copy];

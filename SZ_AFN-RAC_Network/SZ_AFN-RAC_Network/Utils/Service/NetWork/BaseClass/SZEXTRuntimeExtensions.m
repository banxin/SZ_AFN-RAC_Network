...
sz_propertyAttributes *sz_copyPropertyAttributes (objc_property_t property) {
    ...
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate sz_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        switch (flag) {
            ...
            case 'C':
                attributes->memoryManagementPolicy = sz_propertyMemoryManagementPolicyCopy;
                break;
                
            case '&':
                attributes->memoryManagementPolicy = sz_propertyMemoryManagementPolicyRetain;
                break;
